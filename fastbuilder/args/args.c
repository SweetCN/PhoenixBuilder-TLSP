...
struct go_string newAuthServer = {"http://localhost:33250", 22}; // 22 is refer to the length of the URL address of auth server
...
void print_help(const char *self_name) {
  ...
  printf("\t-s, --skip-mcpc-check-challenges: PyRpc packets related "
         "SetStartType and SetMCPCheckNum are blocked from being sent.\n");
  printf("\t-A <url>, --auth-server=<url>: Use the specified authentication "
  ...
  printf("\t-p, --password=<server password>: Specify the password of the "
         "server specified by -c.\n");
  printf("\t-t, --token=<path of FBToken>: Specify the path of FBToken, and "
         "quit if the file is unaccessible.\n");
  printf("\t-T, --plain-token=<token>: Specify the token content.\n");
  printf("\t--no-readline: Suppress user input.\n");
  printf("\t-N, --gamename <name>: Specify the game name to use interactive "
         "commands (e.g. get), instead of using the server provided one.\n");
  printf("\t--ingame-response: Turn on the feature to listen to commands or "
         "give output in game.\n");
  printf("\t--del-userdata: Remove user data and exit.\n");
  printf("\t-L, --listen <address>: Listen and handle WebSocket connection on "
         "given address, this will also suppress readline.\n");
  printf("\n");
...
int _parse_args(int argc, char **argv) {
  while (1) {
    static struct option opts[] = {
        ...
        {"skip-mcpc-check-challenges", no_argument, 0, 's'}, // 15
        {0, 0, 0, 0}};
    int option_index;
    int c = getopt_long(argc, argv, "shA:vc:p:t:T:N:L:", opts, &option_index);
    if (c == -1)
      break;
    switch (c) {
    case 0:
